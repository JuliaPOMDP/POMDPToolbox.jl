...
"""
struct PreviousObservationUpdater{O} <: Updater{Nullable{O}} end
...
Updater that stores the most recent observation as the belief.
"""
struct FastPreviousObservationUpdater{O} <: Updater{O} end
...
"""
struct PrimedPreviousObservationUpdater{O} <: Updater{O}
    default::O
end
