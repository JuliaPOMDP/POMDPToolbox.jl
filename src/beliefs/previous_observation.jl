...
type PreviousObservation{O} <: Belief
...
type PreviousObservationUpdater{O} <: BeliefUpdater end

convert_belief{O}(u::PreviousObservationUpdater{O}, b::Belief) = PreviousObservation{O}()
